...
Produces:
  - figures/pca_baseline_labeled.png
  - figures/pca_trajectories_shared.png
  - figures/pca_baseline_post_shared.png
  - figures/delta_pca_biplot.png
  - figures/delta_by_inv_group.png
  - figures/delta_heatmap.png
  - figures/aggregate_portfolio.png
"""
...
LABEL_THESE = {
    "Yibo Zhu 0001", "Ihsan Ayyub Qazi", "Laurent Vanbever", "Alex C. Snoeren",
    ...
    "Vyas Sekar", "Robert Soulé", "Aaron Schulman", "Mohammad Alizadeh",
...
def plot_pca_baseline_labeled(fv, group_idx):
    ...
    for r in fv["researchers"]:
        ...
        sizes_list.append(90 if grp != "Inv-Q2" else 60)
        names_list.append(name)
        groups_list.append(grp)

    for i, (x, y, c, s) in enumerate(zip(pc1_vals, pc2_vals, colors_list, sizes_list)):
        ax.scatter(x, y, c=c, s=s, alpha=0.75, edgecolors="white", linewidth=0.5, zorder=2)
    ...
    for grp in ["Inv-Q1", "Inv-Q2", "Inv-Q3", "Inv-Q4"]:
        legend_elements.append(
            plt.Line2D([0], [0], marker="o", color="w",
                       markerfacecolor=INV_Q_COLORS[grp], markersize=10,
                       label=INV_Q_LABELS[grp]))
    ax.legend(handles=legend_elements, fontsize=9, loc="lower left",
              title="Investigation Groups", title_fontsize=10)

    # PC2: positive = networking-pure (qualifying + other_net load positively);
    #      negative = systems/mobile/AI (systems, mobile, AI_ML load negatively)
    ax.set_xlabel("PC1 (44.8% var.) — Elite-concentrated ← → Broad-networking", fontsize=11)
    ax.set_ylabel("PC2 (22.1% var.) — Systems/mobile/AI-engaged ← → Networking-pure", fontsize=11)
...
def plot_pca_trajectories(fv, group_idx):
    ...
    # Draw arrows and start-point dots
    for r in fv["researchers"]:
        name = r["name"]
        grp = group_idx.get(name, "excluded")
        if grp == "excluded":
        ...
        y1 = -y1  # shared PC2 is sign-flipped vs baseline PCA
        y2 = -y2

        color = INV_Q_COLORS.get(grp, "#95A5A6")
        is_labeled = name in LABEL_THESE
        lw = 3.0 if is_labeled else 1.8
        alpha = 0.75 if is_labeled else 0.50

        # Start dot
        ax.scatter(x1, y1, c=color, s=20 if not is_labeled else 45,
                  alpha=0.45, zorder=2, edgecolors='none')

        # Arrow
        ax.annotate("", xy=(x2, y2), xytext=(x1, y1),
                    arrowprops=dict(arrowstyle="->", color=color,
                                   lw=lw, alpha=alpha, mutation_scale=20))

    # Labels at POST position
    for r in fv["researchers"]:
        name = r["name"]
        grp = group_idx.get(name, "excluded")
        ...
        x2, y2 = pca_xy(pca_post)
        y2 = -y2
        ax.annotate(name, (x2, y2), fontsize=7, alpha=0.95,
                   xytext=(6, 6), textcoords="offset points",
                   color=INV_Q_COLORS.get(grp, "#333"), fontweight="bold")

    legend_elements = []
    for grp in ["Inv-Q1", "Inv-Q2", "Inv-Q3", "Inv-Q4"]:
        legend_elements.append(
            plt.Line2D([0], [0], color=INV_Q_COLORS[grp], linewidth=2.5,
                       label=INV_Q_LABELS[grp]))
    ...
    ax.set_ylabel("PC2 — Systems/mobile/AI-engaged ← → Networking-pure", fontsize=11)
...
def plot_delta_by_inv_group(fv, group_idx):
    ...
    ax_map = {"Inv-Q1": (0, 0), "Inv-Q2": (0, 1), "Inv-Q3": (1, 0), "Inv-Q4": (1, 1)}
...
def plot_delta_heatmap(fv, group_idx):
    ...
    row_height = 0.30
    fig, ax = plt.subplots(figsize=(14, max(20, n_researchers * row_height)))
    vmax = max(abs(matrix.min()), abs(matrix.max()), 5)
    im = ax.imshow(matrix, aspect="auto", cmap="RdBu_r", vmin=-vmax, vmax=vmax,
                   interpolation="nearest")

    for i, c in enumerate(group_colors):
        ax.add_patch(plt.Rectangle((-0.8, i - 0.5), 0.4, 1, color=c,
                                   clip_on=False, transform=ax.transData, linewidth=0))
    ...
    ax.set_yticklabels([n if n in LABEL_THESE else "" for n in names], fontsize=7)
    ax.set_title("Delta Profiles: Post-2023 − Baseline (pp), by Investigation Group", fontsize=14)

    cbar = plt.colorbar(im, ax=ax, shrink=0.85)
    cbar.set_label("Percentage-point change", fontsize=11)
...
def plot_delta_pca_biplot(fv, group_idx):
    ...
    for r in fv["researchers"]:
        ...
        pc2_vals.append(y)
        colors_list.append(INV_Q_COLORS.get(grp, "#95A5A6"))
        sizes_list.append(80 if grp != "Inv-Q2" else 55)
        names_list.append(name)
        groups_list.append(grp)

    for x, y, c, s in zip(pc1_vals, pc2_vals, colors_list, sizes_list):
        ax.scatter(x, y, c=c, s=s, alpha=0.7, edgecolors="white", linewidth=0.3, zorder=2)
...
# =========================================================================
# 6. Aggregate Portfolio (with delta annotations)
# =========================================================================
def plot_aggregate_portfolio(fv, group_idx=None):
    ...
    width = 0.30
    bl_vals = [bl_means[f] for f in FAMILIES]
    post_vals = [post_means[f] for f in FAMILIES]
    deltas = [post_means[f] - bl_means[f] for f in FAMILIES]

    ax.bar(x - width/2, bl_vals, width, label="Baseline (2018-2022)",
           color="#3498DB", alpha=0.85, edgecolor="white", linewidth=0.5)
    ax.bar(x + width/2, post_vals, width, label="Post-2023 (2023-2026)",
           color="#E74C3C", alpha=0.85, edgecolor="white", linewidth=0.5)

    for i, (dx, pv) in enumerate(zip(deltas, post_vals)):
        color = "#27AE60" if dx > 0 else "#E74C3C"
        label_y = max(bl_vals[i], post_vals[i]) + 1.5
        ax.annotate(f"{dx:+.1f}pp", (x[i], label_y),
                   fontsize=9, fontweight="bold", color=color, ha="center", va="bottom")

    ax.set_xticks(x)
    ax.set_xticklabels(FAMILY_SHORT, rotation=35, ha="right", fontsize=10)
...
def main():
    ...
    plot_aggregate_portfolio(fv)

    quadrant_file = FIGURES_DIR / "delta_by_quadrant.png"
    if quadrant_file.exists():
        print(f"\nNOTE: {quadrant_file} uses old sys/AI/storage quadrant labels (deprecated).")
