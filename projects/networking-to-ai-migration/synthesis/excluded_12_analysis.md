...
|-----------|:--------:|:----------:|:-----------------:|----------------------|-------|
| Ming Zhang 0005 | 12 | 11 | mostly_senior | North China University of Technology | Possible DBLP name collision (common Chinese name "Ming Zhang"); the affiliation does not match the networking researcher profile. May be a PID disambiguation artifact. |
...
Both are classified `inactive_after_2022` and have zero post-2023 papers at any venue. Ming Zhang 0005's identity is suspect (the "0005" DBLP disambiguation number suggests multiple candidates; the North China University of Technology is unlikely to be the correct identity for the networking researcher who published 11 top-networking papers).
...
| Matt Calder | 14 | 12 | 3 | 3 (2 IMC, 1 HotNets) | mostly_collaborator | mostly_collaborator | Unknown | Still 100% networking — just fewer papers. All 3 post papers at top networking venues. May be a data completeness issue (missing papers). |
...
- **Industry exit**: At least 3 of 12 (Hongqiang Harry Liu at Uber AI, Ankit Singla at Google, possibly Ming Zhang 0005) are clearly industry researchers. Industry publication norms differ from academia, and low conference output may not reflect research inactivity.
...
- **Possible data issues**: Ming Zhang 0005's affiliation suggests a DBLP identity collision. Hari Balakrishnan's affiliation is confirmed wrong. Matt Calder's identity has no geo data at all. These data quality issues may affect interpretation.
...
1. **The exclusion threshold hides industry transitions.** Ankit Singla and Hongqiang Harry Liu are at Google and Uber — they are not "leaving networking" as much as moving to industry research roles where conference publishing is deprioritized.
...
1. **Manual verification**: Check DBLP PIDs for Ming Zhang 0005 and Hari Balakrishnan for identity errors.
